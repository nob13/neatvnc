...
{
	size_t outlen;
	char* got = utf8_to_latin1(in, inlen, &outlen);
	assert(got);
	assert(outlen == explen);
	assert(memcmp(got, exp, explen) == 0);
	free(got);
}
...
int main()
{
	...
	test_latin_to_utf8_and_back("ABC \xc4\xd6\xdc\xe4\xf6\xfc\xdf", "ABC ÄÖÜäöüß");
